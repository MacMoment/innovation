...
print_final_instructions() {
    ...
    echo "3. Install the Minecraft Plugin:"
    if [ "$MAVEN_AVAILABLE" = true ] && [ -f "$SCRIPT_DIR/minecraft-plugin/target/StaffSystem-1.0.0.jar" ]; then
        echo "   Copy: minecraft-plugin/target/StaffSystem-1.0.0.jar"
    else
        echo "   Build first: cd minecraft-plugin && mvn clean package"
        echo "   Then copy: target/StaffSystem-1.0.0.jar"
    fi
    echo "   to your server's plugins/ folder"
    echo ""
    
    echo "4. Configure the Minecraft Plugin:"
    echo "   After first server start, edit: plugins/StaffSystem/config.yml"
    echo "   Use values from: minecraft-plugin/generated/config-snippet.yml"
    echo ""
